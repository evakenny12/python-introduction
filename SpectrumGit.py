...
def _(load_data):
    ...
    def rough_plot(wavelength, intensity):
        ...
        ax.plot(
            ...
            wavelength, intensity, "black", marker="o", markersize=1.5, linestyle="none", label="Points" 
        )

        ax.set_xlim([wavelength.min(), wavelength.max()]) # FIXME: set useful limits
        ax.set_ylim([intensity.min(), intensity.max()])
...
def _(curve_fit, load_data):
    ...
    def fit_gaussian():
        p0 = [12] # FIXME: add initial guess for the parameters

        wavelength, intensity = load_data("signal.txt")

        p_opt, p_cov = curve_fit(wavelength, intensity) # FIXME: call curve_fit() with correct arguments
...
@app.cell(hide_code=True)
def _(fit_gaussian, gaussian, load_data, np):
    def pass_fitting():
        passed = False
