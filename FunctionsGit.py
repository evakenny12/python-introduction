...
def _():
    ...
    def get_energy(_electron_mass):
        print(f"LIGHT_SPEED inside the function: {LIGHT_SPEED}")
        return _electron_mass * LIGHT_SPEED ** 2

    _electron_mass = 9.10e-31
    _electron_energy = get_energy(_electron_mass)

    try:
        print(_electron_mass)
...
def _():
    ...
    # We can call the function just like before. Using print_kelvin('temperature here')
    print_kelvin(temp=45)
    print_kelvin(temp=body_temp)

    # We can omit the argument names. (got rid of 'temp=')
    print_kelvin(20)
...
def _(body_temp, body_ten, math):
    ...
    print(get_kelvin(body_temp))
    print(gibbs(18e-5, body_ten))
    return get_kelvin, gibbs
...
def _():
    ...
    class Point:
        ...
        def distance_from(
            ...
            diff_x=self.x-point.x
            diff_y=self.y-point.y
            diff_z=self.z-point.z
            #here self is _point1 and point is _point2
...
def _():
    ...
    def max_density(solvents):
        return max(solvents, key=lambda y: y[2])
...
def _():
    ...
    print(
        ...
        f"{max(
            _solvents, key=lambda x: x.density).name
        }."
    )
    return
